    ...
    { 
        ...
        var ret = $.ajax({
            ...
            success: function (result) 
            {
                ...
                //这里需要加个数据装换方法（康文洋）；
            	result = $.AceResule.getAceResult(result);
...
function ViewModelBase() {
    ...
    me.CopyAndPaste = function () {
        $ace.confirm("确定要复制粘贴该条数据吗？", me.OnCopyAndPaste);
    }
    
    me.OnDelete = function () {
        DeleteRow();
    }
    me.OnCopyAndPaste = function () {
    	CopyAndPasteRow();
    }
    ...
    /* 要求每行必须有 Id 属性 */
    function CopyAndPasteRow() {
        if (me.CopyAndPasteUrl == null)
            throw new Error("未指定 CopyAndPasteUrl");

        var url = me.CopyAndPasteUrl;
        var params = { ids: me.DataTable.SelectedModel().id };
        $ace.post(url, params, function (result) {
            var msg = result.Msg || "复制粘贴成功";
            $ace.msg(msg);
            //me.DataTable.RemoveSelectedModel();
            me.LoadModels();
        });
